      ...
      {receipt ? (
        <>
          <div className="receipt-chain">
            ...
            <ReceiptStep number="04" title="Shadow execution"><KV label="Mode" value={receipt.provenance.executionMode.toUpperCase()} /><KV label="Fill" value={receipt.execution ? `${receipt.execution.details.side} ${receipt.execution.details.size} @ ${receipt.execution.details.price}` : "No fill for this decision"} /><KV label="PnL after" value={receipt.execution?.pnlSnapshot ? String(receipt.execution.pnlSnapshot.pnl) : "—"} /></ReceiptStep>
          </div>
